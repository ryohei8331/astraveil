    ...
    if (S.step === 'main') {
      ...
      btn(ctx, bx, y, bw, 38, '操作方法', () => {
        G.dialog.open('操作マニュアル', [
          ...
          '【攻撃】J か 左クリック。近くの敵に自動で向き直り、連打で3段コンボ。振り中に押せば次の攻撃が予約される。',
          '【回避】K / Shift。出始めに無敵時間があり、敵の攻撃に重ねると「見切り」が発生する。',
          '【魔法】L を長押しでチャージ、離して発動。リングが内側の輪に重なった瞬間に離すと威力1.5倍。Uで属性切替。',
          '【スキル】1〜4。パッシブはセットするだけで常時発動。',
          '【その他】E: 調べる/話す(頭上に▼が出る)。M: メニュー開閉。H: HP20%以下でSOS救援。',
          'ゲーム開始後も、画面の段階式ガイドが1つずつ教えてくれる。',
        ]);
      }, { size: 13 }); y += 46;
